...
Unified parameterized version of the six per-model 3b_<model>.py scripts.
Pass *oracle_model* to ``main()`` to select which model runs the oracle.
...
# ---------------------------------------------------------------------------
# Path resolution — uses triex.config when available, falls back to cwd.
# ---------------------------------------------------------------------------

try:
    from triex.config import results_json as _results_json, exp_dir as _exp_dir
    _EXP_ID = "exp3b_window"
    RESULTS_PATH = str(_results_json(_EXP_ID))
    _EXP_OUT_BASE = str(_exp_dir(_EXP_ID))
except ImportError:
...
# Cap per (player, trait) in per_trait mode, or per player in aggregate mode.
...
DISCREPANCY_MODE = "per_trait"  # or "aggregate"
...
def call_oracle_openai(
    ...
    print(f"Oracle outputs saved: {out_jsonl}  |  N={len(results)}  Fail={n_fail}")
...
# ---------------------------------------------------------------------------
# Per-model worker count (qwen has stricter rate limits)
# ---------------------------------------------------------------------------

_QWEN_MAX_WORKERS = 10
_DEFAULT_MAX_WORKERS = 16


def _model_max_workers(oracle_model: str) -> int:
    return _QWEN_MAX_WORKERS if "qwen" in oracle_model.lower() else _DEFAULT_MAX_WORKERS


def _model_out_dirname(oracle_model: str) -> str:
    """Derive a short output directory name from an OpenRouter model id.
    ...
    """
    return oracle_model.split("/")[-1] if "/" in oracle_model else oracle_model


def main(oracle_model: str = "openai/gpt-4.1-mini") -> None:
    """Run the full exp3b pipeline for *oracle_model*.

    Parameters
    ----------
    oracle_model:
        OpenRouter model id, e.g. ``"deepseek/deepseek-v3.2"`` or
        ``"openai/gpt-4.1-mini"``.
    """
    out_dir = os.path.join(_EXP_OUT_BASE, _model_out_dirname(oracle_model))
    ...
    call_oracle_openai(
        ...
        oracle_out,
        oracle_model=oracle_model,
        max_workers=_model_max_workers(oracle_model),
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run exp3b for a single oracle model.")
    parser.add_argument(
        "--model",
