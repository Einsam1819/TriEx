...
# Global cap on concurrent oracle requests in flight.
INFLIGHT_LIMIT = 6   # 4-8 recommended; 6 is usually stable for qwen3-32b
_inflight_sem = threading.Semaphore(INFLIGHT_LIMIT)
...
def _is_retryable(err: Exception) -> bool:
    s = str(err).lower()
    retry_keys = [
        "429", "rate", "overloaded", "timeout", "timed out", "temporarily",
...
def _load_done_ids(out_jsonl: str) -> set:
    """Resume support: read existing output jsonl and collect completed sample_ids."""
    done = set()
    ...
    try:
        with open(out_jsonl, "r", encoding="utf-8") as f:
            for line in f:
                ...
                r = json.loads(line)
                sid = r.get("sample_id")
                # Treat as done if align_score exists OR a prior error was recorded
                # (otherwise we'd retry the same broken sample forever).
                oracle = r.get("oracle", {}) or {}
                if sid and ("error" in r or oracle.get("align_score", None) is not None):
                    done.add(sid)
    except Exception:
        pass
    return done
...
def call_oracle_openai(
    samples_jsonl: str,
    out_jsonl: str,
    oracle_model: str = "qwen/qwen3-32b",
    max_workers: int = 10,          # do not exceed 10
    max_retries: int = 5,           # generous retries beat hangs
    sleep_base: float = 0.8,
    request_timeout: float = 60.0,  # per-request timeout in seconds
) -> None:
    """Threaded OpenRouter oracle runner with in-flight semaphore, per-request
    timeout, exponential-backoff retries with jitter, and resume support.
    ...
    # Pass timeout to the OpenAI client so it applies per request.
    client = OpenAI(
    ...
    print(f"[oracle] total={len(samples)} done={len(done_ids)} pending={len(pending)}")
    if not pending:
        print("[oracle] Nothing to do (all done).")
        return

    fo = open(out_jsonl, "a", encoding="utf-8")
    ...
    def worker(sample: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            # Exponential backoff with up to 0.25s of jitter.
            if attempt > 0:
                jitter = random.random() * 0.25
                time.sleep((sleep_base * (2 ** (attempt - 1))) + jitter)
            ...
            try:
                ...
                last_err = str(e)
                # Stop early on non-retryable errors after the first attempt.
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futs = [ex.submit(worker, s) for s in pending]
        for fut in as_completed(futs):
            r = fut.result()
            write_row(r)
            processed += 1
            if r.get("oracle", {}).get("align_score", None) is None:
                fail += 1
            if processed % 10 == 0:
                print(f"[oracle] processed {processed}/{len(pending)}  fail={fail}")

    fo.close()
    print(f"[OK] Oracle outputs appended: {out_jsonl} | Pending={len(pending)} Fail={fail}")
...
def main():
    ...
    print(f"[OK] Built {len(rows)} samples (mode={DISCREPANCY_MODE}, window_k={WINDOW_K})")
