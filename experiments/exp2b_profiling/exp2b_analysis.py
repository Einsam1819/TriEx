"""Experiment 2B profiling: trait-metric correlation analysis.

This is the importable (entrypoint-compatible) version of 2B_analysis.py,
with all execution logic wrapped in ``main()`` and paths resolved via
``triex.config`` when available.
"""
...
try:
    ...
    RESULTS_PATH = str(_rj("exp2b_profiling"))
    _OUT_DIR = str(_od("exp2b_profiling"))
except ImportError:
...
def main():
    ...
    float_cols = ["spearman_r", "spearman_p", "pearson_r", "pearson_p"]
    for col in float_cols:
        if col in df_corr.columns:
            df_corr[col] = df_corr[col].round(4)

    os.makedirs(_OUT_DIR, exist_ok=True)
    out_csv = os.path.join(_OUT_DIR, "exp2b_alignment_all_traits.csv")
